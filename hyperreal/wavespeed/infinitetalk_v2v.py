...
# One WaveSpeed docs page spells this "infinietalk" — assumed typo; if the first
# live run 404s, this path is the thing to re-check.
MODEL_PATH = "wavespeed-ai/infinitetalk/video-to-video"
...
class WaveSpeedInfiniteTalkV2V(SuccessFailureNode):
    """Video + audio -> relipsynced video via WaveSpeed InfiniteTalk video-to-video."""

    def __init__(self, name: str, metadata: dict[Any, Any] | None = None, **_: Any) -> None:
        node_metadata = {
            "category": "video/wavespeed",
            "description": "Relipsync an existing video to a new audio track via WaveSpeed InfiniteTalk video-to-video.",
        }
        if metadata:
        ...
        self.add_parameter(
            Parameter(
                ...
                tooltip="The base video to relipsync.",
        ...
        self._create_status_parameters(
            result_details_tooltip="Details about the WaveSpeed InfiniteTalk video-to-video result",
    ...
    def _build_body(self, api_key: str) -> dict[str, Any]:
        body: dict[str, Any] = {
            "video": self._artifact_to_url(api_key, self._require("video"), "video"),
